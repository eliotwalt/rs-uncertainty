...
def check_integrity(create_config, predict_config, eval_config):
    ...
    assert create_config["sampling_strategy"]==predict_config["sampling_strategy"]=="s2_offset_valid_center"
...
with open("config/create_dataset/cloud_exp_template.yaml") as f:
    create_config = yaml.safe_load(f)
with open("config/predict_testset/cloud_exp_template.yaml") as f:
    predict_config = yaml.safe_load(f)
with open("config/evaluate_testset/cloud_exp_template.yaml") as f:
...
for imageDir in os.scandir(Path(create_config["s2_reprojected_dir"]).parents[0]):
    td = Path(f"/tmp/cldexp_{imageDir.name}")
